(function(w, doc, undefined) {
   ...
   */
    if (
        'scrollSnapType' in doc.documentElement.style ||
   ...
   */
    function setUpElement(obj, declaration) {
        ...
        var tag = obj.tagName;
        if (tag.toLowerCase() == 'body' || tag.toLowerCase() == 'html') {
            obj = doc;
        }
   ...
   */
    var handlerDelayed = function() {
        ...
        if (
            ...
            scrollStart.x == lastScrollObj.scrollLeft
        ) {
            // ignore timeout
            return;
        }
        ...
        if (
            typeof lastScrollObj.snapElements !== 'undefined' &&
            lastScrollObj.snapElements.length > 0
        ) {
            snapPoint = getNextElementSnapPoint(
                lastScrollObj,
                lastObj,
                direction
            );
   ...
   */
    function getNextSnapPoint(scrollObj, obj, direction) {
        ...
        nextPoint.y = constrainByDistance(
            initialPoint.y,
            currentPoint.y,
            nextPoint.y,
            scrollStart.y,
        ...
        nextPoint.x = constrainByDistance(
            initialPoint.x,
            currentPoint.x,
            nextPoint.x,
            scrollStart.x,
    ...
    function getNextElementSnapPoint(scrollObj, obj, direction) {
        ...
        for (
            var i = currentIteration + primaryDirection;
            i < l && i >= 0;
            i = i + primaryDirection
        ) {
            ...
            snapCoords = {
                y:
                    currentIteratedObj.offsetTop -
                    scrollObj.offsetTop +
                    getYSnapLength(
                        currentIteratedObj,
                        currentIteratedObj.snapLengthUnit.y
                    ),
                // & obj.left + obj.snaplength.x
                x:
                    currentIteratedObj.offsetLeft -
                    scrollObj.offsetLeft +
                    getXSnapLength(
                        currentIteratedObj,
                        currentIteratedObj.snapLengthUnit.x
                    )
            };
            ...
            if (
                left <= snapCoords.x &&
                left + getWidth(scrollObj) >= snapCoords.x &&
                top <= snapCoords.y &&
                top + getHeight(scrollObj) >= snapCoords.y
            ) {
                ...
                return {
                    y: stayInBounds(
                        0,
                        getScrollHeight(scrollObj),
                        snapCoords.y - snapDest.y
                    ),
                    x: stayInBounds(
                        0,
                        getScrollWidth(scrollObj),
                        snapCoords.x - snapDest.x
                    )
                };
            }
        }
        ...
        return {
            y: stayInBounds(
                0,
                getScrollHeight(scrollObj),
                obj.snapElements[currentIteration].snapCoords.y - snapDest.y
            ),
            x: stayInBounds(
                0,
                getScrollWidth(scrollObj),
                obj.snapElements[currentIteration].snapCoords.x - snapDest.x
            )
        };
    }
   ...
   */
    function constrainByDistance(
        ...
        if (
            Math.abs(scrollStart - currentScrollValue) < MIN_PX_CONSTRAINT &&
            Math.abs(initialPoint - currentPoint) < SNAP_CONSTRAINT &&
   ...
   */
    function parseSnapCoordValue(declaration) {
        ...
        if (typeof declaration['scroll-snap-coordinate'] !== 'undefined') {
            ...
            typeof declaration['scroll-snap-destination'] !== 'undefined'
        ) {
   ...
   */
    function getYSnapLength(obj, declaration) {
        if (declaration.unit == 'vh') {
            ...
            return (
                Math.max(doc.documentElement.clientHeight, w.innerHeight || 1) /
                100 *
                declaration.value
            );
   ...
   */
    function getXSnapLength(obj, declaration) {
        if (declaration.unit == 'vw') {
            ...
            return (
                Math.max(doc.documentElement.clientWidth, w.innerWidth || 1) /
                100 *
                declaration.value
            );
        } else if (declaration.unit == '%') {
   ...
   */
    var smoothScroll = function(obj, end, callback) {
        var start = { y: obj.scrollTop, x: obj.scrollLeft },
            ...
            duration = Math.max(
                getDuration(start.y, end.y),
                getDuration(start.x, end.x)
            );
